...
$(window).on("keydown", debounce((e) => {
    ...
    switch (e.keyCode) {
        ...
        case 40:
            // ArrowDown
            if (options.axisOptions.yAxis.max - 500 > 0) {
                options.axisOptions.yAxis.max -= 500;
            }
            break;
    }
...
if(demoAPI = prompt("호출할 API 주소를 아래와 입력해 주세요. :)", "http://example.com/start_time=${startTime}&end_time=${endTime}")){
    setInterval(() => {
        var url: string = demoAPI;
        url = url.replace("${startTime}", startTime.toString());
        url = url.replace("${endTime}", endTime.toString());
        $.ajax({
            url: url
